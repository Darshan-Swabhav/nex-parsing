...
const naicsCodes = require('@nexsalesdev/master-data-model/lib/dataFiles/naicsCode.json');
const {
  loggerMock
} = require('../../../../../helper');
...
const naicsCodeControllerModule = proxyquire(
  '../../../../../../controllers/v1/master/search/naicsCodeController', {
    '../../../../services/master/search/autoCompleteService': AutoCompleteServiceStub,
  }
);

describe('#naicsCodeController - get', function () {
  describe('Get All NAICS Code', function () {
    context('Check If User is Unauthorized', function () {
      it('Should return `401` with `Unauthorized` error', function (done) {
        ...
        //Act
        naicsCodeControllerModule.get(settingsConfig, req, res, next)
          .then(function () {
    ...
    context('Check if correct params are passed for getting list of naics code', function () {
      before(function () {
        autoCompleteServiceInstanceStub.search = sinon.stub().returns(naicsCodes);
      });
      it('Should return `200` with master naics code list', function (done) {
        // Arrange
        const next = function (error, result) {
          if (error) throw error;
          return result;
        ...
        naicsCodeControllerModule.get(settingsConfig, req, res, next)
          .then(function (result) {
            ...
            const actualData = result.data;
            const actualStatusCode = result.statusCode;
            const expectedData = JSON.parse(JSON.stringify(naicsCodes));
